...
static unsigned int str2ulong(unsigned char *str)
{
   return ( str[0] | (str[1]<<8) | (str[2]<<16) | (str[3]<<24) );
}
static unsigned int str2ushort(unsigned char *str)
{
...
static int avi_add_index_entry(avi_t *AVI, unsigned char *tag, int flags, unsigned int pos, unsigned int len)
{
   void *ptr;
...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  int header_offset=0, hdrl_len=0;
  int nvi, nai[AVI_MAX_TRACKS], ioff;
  int tot[AVI_MAX_TRACKS];
  int j;
  int lasttag = 0;
